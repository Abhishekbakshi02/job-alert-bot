...
Requirement 2 (Location): the role must be EITHER fully remote and open to candidates globally, OR based in India. If the job description states that the role is in another country but provides VISA application support, visa sponsorship, or work visa assistance, ACCEPT the role and include it as well. Trust each job's "Official Listed Location" as the source of truth - use its description only to add detail, not to override it. Reject roles if role is outside india and visa assistance not provided or is not completely remote.
