...
limited or unavailable, automatically falls through to the next, so
one provider's low daily quota doesn't stall the whole pipeline.

To add another provider later: add an entry to PROVIDERS with the same
shape. Most providers (Groq, Gemini, OpenRouter, Mistral, Cerebras, ...)
speak this same OpenAI-compatible format.
"""
